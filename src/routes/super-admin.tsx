...
function SuperAdminPage() {
  ...
  if (!ready || loading) {
    return (
      <div className="p-8 text-sm text-muted-foreground">Loading super-admin dashboard…</div>
    );
  }
      ...
      <main className="mx-auto max-w-7xl px-6 py-8 space-y-8">
        ...
        <section className="rounded-lg border bg-card p-5">
          ...
          <form onSubmit={onCreate} className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
            ...
            <input
              className="rounded border px-3 py-2 text-sm"
              placeholder="APMC License"
              value={newLicense}
              onChange={(e) => setNewLicense(e.target.value)}
            />
            <button
              disabled={creating}
              className="md:col-span-4 rounded bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60"
            >
